module ImplementationSpecHelpers
  ...
  def expect_default_scenario_to_be_defined_for_scope(scope, defining_method)
    ...
    expect(mock_double).to receive(defining_method).with(method_name, anything) do |name, block|
